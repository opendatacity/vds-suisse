...
exports.import = function (cells, vds, config) {
	...
	vds.forEach(function (entry) {
		...
		var i0 = Math.round((entry.timeStart - config.timeStart)/config.timeInterval);
		var i1 = Math.round((entry.timeEnd   - config.timeStart)/config.timeInterval);

		if ((i0 < 0) || (i1 > config.indexCount)) console.error('Außerhalb des Zeitintervalls');
	...
	function setCell(index, cellId) {
		...
		if (activities[index] === undefined) {
			var time = index*config.timeInterval + config.timeStart;
			activities[index] = {
				index: index,
				time: time,
